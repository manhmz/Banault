...
export class RemoteSignService {
  ...
  checkSignBlock(stringdata: string) {
    try {
      ...
      return (this.util.account.isValidAccount(data.block.account) &&
        ...
        this.util.banano.isValidHash(data.block.previous) &&
        (data.previous ? this.util.banano.isValidHash(data.previous.previous) : true) &&
        this.util.banano.isValidHash(data.block.link) &&
        (data.previous ? this.util.banano.isValidHash(data.previous.link) : true) &&
        (data.previous ? this.util.banano.isValidSignature(data.previous.signature) : true));
    } catch (error) {
      return false;
    }
  }
  ...
  checkProcessBlock(stringdata: string) {
    try {
      const data = JSON.parse(stringdata);
      return (this.util.banano.isValidSignature(data.block.signature) &&
        (data.block.work ? this.util.banano.isValidWork(data.block.work) : true));
    } catch (error) {
      return false;
    }
  }
